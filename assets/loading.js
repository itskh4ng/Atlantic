document.addEventListener("DOMContentLoaded", function () {
    ...
    function animate() {
        if (growing) {
            svg.style.transform = `translate(-50%, -50%) scale(1.67) rotate(${rotating}deg)`;
        } else {
            svg.style.transform = `translate(-50%, -50%) scale(1) rotate(${rotating}deg)`;
        }
        rotating += 20;
        growing = !growing;
